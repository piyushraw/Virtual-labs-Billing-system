...
def user_profile(request):
    context1 = {}
    context1['products'] = Product.objects.filter(user=request.user)
    context = {}
    user_profile = get_object_or_404(UserProfile, user=request.user)
...
def invoice_create(request):
    context2 = {}
    context2['products'] = Product.objects.filter(user=request.user)
...
def TAbills(request):
    return render(request,'gstbillingapp/TAbills.html')
def DAbills(request):
    return render(request,'gstbillingapp/DAbills.html')
